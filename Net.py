...
class FusionNet_RSFI(nn.Module):
    def __init__(self, output=1, img_zie=(480, 640)):
        super(FusionNet_RSFI, self).__init__()
        vis_ch = [8, 16, 24, 32, 48]
        inf_ch = [8, 16, 24, 32, 48]
        growth_rate = [4, 8]
        output = 1
        ...
        ## global feature
        self.vis_conv2 = ConvLeakyRelu2d(1, 3)
        self.vis_glb = GFE(img_zie)
        self.inf_conv2 = ConvLeakyRelu2d(1, 3)
        self.inf_glb = GFE(img_zie)
...
class WO_ViT(nn.Module):
    def __init__(self, output=1, img_zie=(480, 640)):
        super(WO_ViT, self).__init__()
        vis_ch = [8, 16, 24, 32, 48]
        inf_ch = [8, 16, 24, 32, 48]
        growth_rate = [4, 8]
        output = 1
...
class Ab_MSFF(nn.Module):

    def __init__(self, channels):
        super(Ab_MSFF, self).__init__()
    ...
    def forward(self,x_k3, x_k5, x_k7):
...
class WO_MSFF(nn.Module):

    def __init__(self, output=1, img_zie=(480, 640)):
        super(WO_MSFF, self).__init__()
        vis_ch = [8, 16, 24, 32, 48]
        inf_ch = [8, 16, 24, 32, 48]
        growth_rate = [4, 8]
        output = 1
        ...
        self.MSFF_vis = Ab_MSFF(vis_ch[2])
        self.MSFF_inf = Ab_MSFF(inf_ch[2])
...
class Ab_CMFF(nn.Module):

    def __init__(self, all_channel, global_and_local=True):
        super(Ab_CMFF, self).__init__()
        ...
        self.conv1 = nn.Conv2d(all_channel, all_channel, kernel_size=3, padding=1)

        self.conv2 = nn.Conv2d(all_channel, all_channel, kernel_size=3, padding=1)
    ...
    def forward(self, x, y):
        ...
        out = F.leaky_relu(x + y)

        return out

class WO_CMFF(nn.Module):

    def __init__(self, output=1, img_zie=(480, 640)):
        super(WO_CMFF, self).__init__()
        vis_ch = [8, 16, 24, 32, 48]
        inf_ch = [8, 16, 24, 32, 48]
        growth_rate = [4, 8]
        output = 1
        ...
        self.fuse_vis = Ab_CMFF(inf_ch[2])
        self.fuse_inf = Ab_CMFF(inf_ch[2])
        self.fuse = Ab_CMFF(inf_ch[2], global_and_local=False)
        '''###################################### Decoder'''
        self.decode4 = ConvBnLeakyRelu2d(vis_ch[2], vis_ch[1] + vis_ch[1])
        self.decode3 = ConvBnLeakyRelu2d(vis_ch[1] + inf_ch[1], vis_ch[0] + inf_ch[0])
        self.decode2 = ConvBnLeakyRelu2d(vis_ch[0] + inf_ch[0], vis_ch[0])
...
class WO_SFI(nn.Module):

    def __init__(self, output=1, img_zie=(480, 640)):
        super(WO_SFI, self).__init__()
        vis_ch = [8, 16, 24, 32, 48]
        inf_ch = [8, 16, 24, 32, 48]
        growth_rate = [4, 8]
        output = 1
...
if __name__ == '__main__':
    ...
    visibel = torch.randn(1, 3, 768, 1024)
    infrared = torch.randn(1, 1, 768, 1024)
    mask = torch.randn(1, 9, 768, 1024)
    model = FusionNet(img_size=(768, 1024),ViTmodel = 'vit_light_patch16_F')
    visibel = visibel.to(device)
    infrared = infrared.to(device)
    mask = mask.to(device)
    model = model.to(device)
    print(model(visibel, infrared, mask).shape)
